#!/bin/bash
# HIS-Go 管理端演示启动脚本
# 用法: ./scripts/demo-admin.sh [start|stop|restart|status|logs]
...
start_services() {
    ...
    echo "  - Clinic: http://localhost:8084"
    echo "  - Prescription: http://localhost:8085"
    echo "  - Billing: http://localhost:8086"
    echo "  - Pharmacy: http://localhost:8087"
    echo "  - Schedule: http://localhost:8090"
    echo "  - System: http://localhost:8096"
    echo "  - Nginx: http://localhost:80"
    echo ""
    print_info "管理端访问地址: http://localhost/admin"
...
stop_services() {
    ...
    print_success "管理端演示服务已停止"
}

# 重启服务
restart_services() {
    print_info "重启管理端演示服务..."
    stop_services
    start_services
}
