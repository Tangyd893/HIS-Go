#!/bin/bash
# HIS-Go 演示验证工具
# 用法: ./scripts/demo-verify.sh [command]
...
verify_admin() {
    ...
    echo ""
    print_info "验证结果: $passed/$((total+1)) 项通过"
    
    if [ $failed -eq 0 ]; then
        print_success "管理端演示环境验证通过！"
        return 0
    else
        print_error "管理端演示环境验证失败，共 $failed 项异常"
        return 1
    fi
}
...
verify_patient() {
    ...
    if [ $failed -eq 0 ]; then
        ...
        print_error "患者端演示环境验证失败，共 $failed 项异常"
        return 1
    fi
}
...
test_api() {
    ...
    if echo "$login_response" | jq -e '.code' > /dev/null 2>&1; then
        local code=$(echo "$login_response" | jq -r '.code')
        if [ "$code" = "200" ]; then
            ...
            if echo "$user_response" | jq -e '.code' > /dev/null 2>&1; then
                local user_code=$(echo "$user_response" | jq -r '.code')
                if [ "$user_code" = "200" ]; then
                    print_success "获取用户信息接口测试通过"
                else
                    print_error "获取用户信息接口测试失败"
                    return 1
                fi
            else
